  ...
  <app-dialog ref="dialogRef" persistent>
      ...
      <div class="row items-center q-gutter-md">
        ...
        <div class="column">
          ...
          <span class="text-caption text-grey-7">
            {{ $t('common.actions.category.editDescription') }}
          </span>
        </div>
      </div>
    </template>
    ...
    <template #form>
      <q-form @submit.prevent.stop="onSubmit">
        ...
        <span class="text-body2">{{ t('common.fields.description') }} *</span>
        <q-input
          dense
...
} from 'src/schemas/category.schemas';
...
async function onSubmit() {
  ...
  try {
    ...
    await handle(
      () => updateCategory(data),
      t('common.feedback.category.updated')
    );
    onDialogOK();
  } catch (err) {
